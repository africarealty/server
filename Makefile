...
# service code
SERVICE = africarealty

# current version
DOCKER_TAG ?= latest
# docker registry url
DOCKER_URL = dockerhub.com/africarealty/server
...
# infrastructure =======================================================================================================
init-infra:
	@docker network create --driver bridge ar > /dev/null; \
...
check-infra:
	docker container ls -a --format 'table {{.ID}}\t{{.Names}}{{.Image}}\t{{.Status}}\t{{.Ports}}\t{{.RunningFor}}' | grep -e 'ar-'

# aerospike ============================================================================================================
aql:
	docker exec -it ar-aerospike bash -c aql
...
swagger:
	...
	swag init -d ./src/cmd,./src/http/auth,./src/http/system,./src/kit/http -o ./src/swagger --parseInternal